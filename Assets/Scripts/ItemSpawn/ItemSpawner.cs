...
public class ItemSpawner : MonoBehaviour
{
    ...
    public GameObject TestSpawnBed(ItemData dataToSpawn, Vector3 spawnPosition, Vector3 spawnScale)
    {
        if (dataToSpawn.itemType == ItemType.Pot)
        {
            ...
            {
                Debug.LogWarning("TrainController не назначен в ItemSpawner. Невозможно определить вагон для грядка. Грядка уничтожено.");
        ...
        if (dataToSpawn.itemType == ItemType.Seed)
        {
            ...
            if (trainController != null)
            {
                ...
                if (parentAssignedSuccessfully)
                {
                    ...
                    if (parentWagon == null)
                    {
                        Debug.LogError($"AssignParentWagonByPosition вернул true, но родитель у {plantObject.name} не установился! растение уничтожено.", plantObject);
                        Destroy(plantObject);
                        return null;
                    }
                ...
                {
                    Debug.LogError($"Не удалось найти или назначить родительский вагон для растения '{plantObject.name}' в позиции {spawnPosition}. растения уничтожено.");
                    Destroy(plantObject);
                    return null;
                }
            ...
            {
                Debug.LogWarning("TrainController не назначен в ItemSpawner. Невозможно определить вагон для растения. растения уничтожено.");
